...
GPS position → procedural nearby fantasy POIs → visit/explore POI → roll encounter reward → gain XP/loot → inventory and character state persist in `localStorage`. A **Collection Log / Codex** tracks discovered POIs, loot, and encounters across sessions in the same save. The in-viewfinder **Expedition** surface groups the current outing report, three short-term Field Contracts, and the latest 50 trail events without changing their underlying save domains. Contracts advance when you explore; completing one grants a small bonus XP reward.
